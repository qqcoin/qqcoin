...
unsigned int nStakeTargetSpacing = 20; //  20 second block spacing
...
const string strMessageMagic = "QQcoin Signed Message:\n";
...
// QQcoin: increasing Nfactor gradually
...
int64 GetProofOfWorkReward( int nHeight, uint256 prevHash)
{
        ...
        int rand0 = generateMTRandom(seed, 2);
        int rand1 = 0;
        int rand2 = 0;
        int rand3 = 0;
        int rand4 = 0;
        ...
        nSubsidy = 1 * COIN;
            ...
            cseed_str = prevHash.ToString().substr(5,7);
                cseed = cseed_str.c_str();
                seed = hex2long(cseed);
                rand2 = generateMTRandom(seed,  400);
                if(rand2 < 40)
                nSubsidy = rand2 + 40 * COIN;
                else if(rand2 > 40)
                nSubsidy = rand2 * COIN;  
            }
        ...
        else if(nHeight < 90000)      
        {
                ...
                cseed = cseed_str.c_str();
                seed = hex2long(cseed);
                rand5 = generateMTRandom(seed, 100);
                if(rand5 < 10)
                nSubsidy = rand5 + 10 * COIN;
                else if(rand5 > 10)
                nSubsidy = rand5 * COIN;
        }
        else if(nHeight < 450000)      
        {
                ...
                cseed = cseed_str.c_str();
                seed = hex2long(cseed);
                rand4 = generateMTRandom(seed, 40);
                if(rand4 < 4)
    ...
    if (!IsCoinBase())
    {
        ...
        if (IsCoinStake())
        {
            ...
            if (nStakeReward > GetProofOfStakeReward(nCoinAge) - GetMinFee() + MIN_TX_FEE)
                return DoS(100, error("ConnectInputs() : %s stake reward exceeded, coin age: %lld, ntime: %lld, value in: %lld, value out: %lld, pos reward: %lld, target pos reward: %lld"
                                     , GetHash().ToString().substr(0,10).c_str(),nCoinAge, nTime, nValueIn, GetValueOut(), nStakeReward, (GetProofOfStakeReward(nTime,nCoinAge) - GetMinFee() + MIN_TX_FEE)));
        }
        else
        {
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "QQcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
  ...
  //vMerkleTree: 05b4eb8ba8 
        ...
        // Genesis block
        const char* pszTimestamp = "QQcoin to the Moon";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0x25b26aaa89d257da4a0a091e3dccecaca990ac0f8691625668152f4b8751690d"));
        block.print();
